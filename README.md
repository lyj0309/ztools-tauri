...
- SQLite WAL 持久化设置、收藏、历史、本地启动项、插件数据和纯文本剪贴板历史；支持数据库与已安装插件的完整备份恢复。
...
- `设置`、`系统` 与 `截图` 三个默认插件直接编译进可执行文件，首次启动和版本升级时自动安装或修复，便携版也无需外置资源目录。
- `截图` 指令直接打开选区编辑器，支持标注、复制、保存和贴图；独立的 `贴图` 指令从本应用记录的剪贴板历史图片中选择原图，生成可拖动、滚轮等比缩放的悬浮贴图。
- 图片历史随剪贴板监控开关启停，保留最近 50 张且原图合计不超过 128 MiB，并遵循历史保留天数；清空剪贴板历史时也会清空图片。升级前未记录的图片不会自动补回。
- 截图默认插件支持鼠标所在显示器选区、矩形/箭头/画笔/文字标注、撤销、保存、图片剪贴板和可缩放置顶贴图。
...
核心插件迁移仍在进行：通用图片/文件剪贴板历史、插件后台模式、基础子窗口、崩溃恢复、旧插件数据迁移和三平台真机验收尚未完成。超级面板、悬浮球、AI、支付和浏览器自动化不在当前核心范围。详细现状见[迁移状态](docs/migration-status.md)，逐项 API 状态见[兼容台账](docs/plugin-api-compatibility.json)，范围与验收门槛见[核心功能迁移方案](docs/core-migration-plan.md)。
...
- 默认插件「百度翻译」提供文字、图片两个入口。图片支持选取 PNG/JPEG/WebP 或直接粘贴，等比缩放、必要补白后提交百度图片翻译 API，显示图片原文和译文。
- 在插件的「接口设置」填写自己的百度翻译开放平台 APPID、密钥，并分别开通通用文本翻译和图片翻译。配置保存在该插件的本地存储中。文字不超过 6000 字节，图片上传不超过 4 MB；费用和额度以百度账户为准。
- 使用官方 HTTPS API；不抓取网页登录 Cookie，不绕过验证码。没有有效凭据时会显示配置或服务错误，不提供虚构译文。
- 截图选区后点击「OCR」或按 `O`，可在原界面查看、编辑和复制识别结果，不上传图片。Windows 调用系统 OCR，依赖 Windows 已安装的文字识别语言包；其他平台调用本机 Tesseract（需自行安装及安装语言包）。
- 官方接口文档：[文本翻译](https://api.fanyi.baidu.com/doc/23)、[图片翻译](https://api.fanyi.baidu.com/doc/26)。
