...
- Rust 后台剪贴板监控、自动粘贴、文件打开/定位、受约束 HTTP 请求、系统通知和屏幕截图。
...
- `设置` 与 `系统` 两个默认插件直接编译进可执行文件，首次启动和版本升级时自动安装或修复，便携版也无需外置资源目录。
...
核心插件迁移仍在进行：图片/文件剪贴板、插件后台模式、基础子窗口、崩溃恢复、旧插件数据迁移和三平台真机验收尚未完成。超级面板、悬浮球、AI、支付和浏览器自动化不在当前核心范围。详细现状见[迁移状态](docs/migration-status.md)，逐项 API 状态见[兼容台账](docs/plugin-api-compatibility.json)，范围与验收门槛见[核心功能迁移方案](docs/core-migration-plan.md)。
